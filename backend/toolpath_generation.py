...
from svgelements import Circle as ElementsCircle, Rect as ElementsRec
from svgpathtools import Path, Line

from path_quadtree import QuadTree, Point, Rect


def circle(cx: float, cy: float, radius: float) -> Path:
    return Path(ElementsCircle(cx, cy, radius).d())


def rect(cx: float, cy: float, width: float, height: float) -> Path:
    return Path(ElementsRec(cx, cy, width, height).d())
...
def get_quadtree_height_intersections(paths: List[Path],
    ...
    lines = {height: Path(Line(complex(-10000, height), complex(10000, height))) for height in heights}
    height_intersections = {height: [] for height in heights}

    # get all intersections
    for height, line in lines.items():
        _, found_paths = quadtree.get_path_collisions(line)
