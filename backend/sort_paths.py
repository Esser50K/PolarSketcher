...
class ClosedPath:
    def __init__(self, path: Path, new_start=0):
        self.path = path
...
def find_closest_path(last_point: complex, paths: list[Path], _: tuple[int, int]) -> (Path, list[Path]):
    """
...
def find_closest_point(last_point: complex, path: Path, step=.05) -> (float, float):
    progress = 0.0
    closest_dist = None
    closest_point = 0
    while progress <= 1.0:
...
def find_closest_path_with_circular_path_check(last_point: complex, paths: list[Path], _: tuple[int, int], step=.05) -> (Path, list[Path]):
    """
...
def find_closest_path_with_radar_scan(last_point: complex, paths: list[Path], size: tuple[int, int], radar_step=2, step=.05) -> (Path, list[Path]):
    """
